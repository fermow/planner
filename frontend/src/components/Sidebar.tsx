...
export default function Sidebar({ hidden = false }: { hidden?: boolean }) {
  ...
  const navItems: { page: Page; label: string; icon: React.ReactNode }[] = [
    ...
    { page: 'calendar', label: t('nav.calendar'), icon: <CalendarDays size={18} /> },
  ...
  return (
    <>
      ...
      <aside className={`${hidden ? 'hidden' : 'hidden md:flex'} fixed left-0 top-0 h-screen w-56 z-40 glass-card rounded-none border-l-0 border-t-0 border-b-0 flex-col`}>
      ...
      <motion.aside
        ...
        className={`${hidden ? 'hidden' : 'md:hidden'} fixed left-0 top-0 h-full w-64 z-50 glass-card rounded-none border-l-0 border-t-0 border-b-0 flex flex-col shadow-2xl`}
      >
