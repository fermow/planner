...
const PageBreak = Node.create({
  ...
  addAttributes() { return { height: { default: null, parseHTML: (element: HTMLElement) => element.getAttribute('data-height'), renderHTML: (attributes: { height?: string }) => attributes.height ? { 'data-height': attributes.height, style: `height:${attributes.height}px` } : {} } }; },
  parseHTML() { return [{ tag: 'div[data-page-break]' }]; },
  renderHTML({ HTMLAttributes }) { return ['div', { ...HTMLAttributes, 'data-page-break': '', class: 'editor-page-break' }]; },
});
...
function Editor({ value, direction, onChange }: { value: string; direction: ResearchDocument['direction']; onChange: (html: string) => void }) {
  ...
  return <div className="word-editor-shell" dir={direction}><div className="word-toolbar" dir="ltr">
    ...
    <span className="word-tool-group">{b('Find and replace', <Search size={15} />, () => setFindOpen((open) => !open), findOpen)}{b('New page', <span className="word-heading">Page</span>, () => { const editorRect = editor.view.dom.getBoundingClientRect(); const cursor = editor.view.coordsAtPos(editor.state.selection.from); const isLetter = editor.view.dom.closest('.word-page')?.classList.contains('page-letter'); const pageRatio = isLetter ? 11 / 8.5 : 297 / 210; const pageHeight = editorRect.width * pageRatio; const used = ((cursor.top - editorRect.top) % pageHeight + pageHeight) % pageHeight; const height = Math.max(48, Math.round(pageHeight - used)); editor.chain().focus().insertContent([{ type: 'pageBreak', attrs: { height } }, { type: 'paragraph' }]).run(); })}</span>
