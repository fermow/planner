...
function Editor({ value, direction, onChange }: { value: string; direction: ResearchDocument['direction']; onChange: (html: string) => void }) {
  ...
  return <div className="word-editor-shell" dir={direction}><div className="word-toolbar" dir="ltr">
    ...
    <span className="word-tool-group">{b('Find and replace', <Search size={15} />, () => setFindOpen((open) => !open), findOpen)}{b('New page', <span className="word-heading">Page</span>, () => editor.chain().focus().insertContent([{ type: 'pageBreak' }, { type: 'paragraph' }]).run())}</span>
